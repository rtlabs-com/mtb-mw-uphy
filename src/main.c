...
#define UPHY_LOOPBACK_TEST

#ifdef UPHY_LOOPBACK_TEST
void uphy_loopback_test(void)
{
    static int update_ctr = 100;
    static uint8_t data_ctr = 0;

    /* write new input value every 100 ticks */
    if (--update_ctr == 0)
    {
      printf("set input data to %d (out is now %d)\n", data_ctr, up_data.O8.Output_8_bits.value);
      up_data.I8.Input_8_bits.value = data_ctr;
      data_ctr++;
      update_ctr = 100;
 	}
}
